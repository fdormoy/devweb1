...
SELECT client.prenom, client.nom, commande.date_achat, commande_id, ROUND(SUM(prix_total),2)
FROM `commande_ligne` 
...
SELECT YEAR(date_achat) AS annee, 
MONTH(date_achat) AS mois, 
ROUND(SUM(cache_prix_total),2) AS montant_global
FROM commande
GROUP BY annee, mois
ORDER BY annee, mois;

--Obtenir la liste des 10 clients qui ont effectué le plus grand montant 
--de commandes, et obtenir ce montant total pour chaque client.
SELECT client.nom, client.prenom, 
ROUND(SUM(commande.cache_prix_total), 2) AS montant_commande_total
FROM commande 
...
SELECT date_achat, ROUND(SUM(cache_prix_total), 2) AS montant_total_commande
FROM commande
GROUP BY date_achat;
